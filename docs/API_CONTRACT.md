...
| Timetable | `GET/POST /timetable`, `GET/DELETE /timetable/{id}`, `PATCH /timetable/{id}/room`, `POST /timetable/import` | Institutional timetable management |
...
- Legacy optimizer/clash/history endpoints preserve their tested nested JSON structures. Frontend integration must not rename their keys; use the committed OpenAPI path definitions plus live integration tests when wiring these operational screens.
