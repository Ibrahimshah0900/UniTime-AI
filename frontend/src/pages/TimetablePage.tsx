import { CalendarRange, Clock3, FileUp, List, PencilLine, Plus, Search, Trash2 } from 'lucide-react'
...
import { EmptyState, ErrorNote, ErrorState, LoadingState, Modal, PageHeader, Section, StatusBadge, SuccessNote } from '../components/Ui'
...
export function TimetablePage() {
  ...
  return <div className="page"><PageHeader title={user?.role === 'student' ? 'My timetable' : user?.role === 'faculty' ? 'Teaching timetable' : 'Timetable management'} description={manager ? 'Manage the institutional timetable, imports and room assignments.' : 'Your schedule is derived from the assignments mapped to your account.'} actions={<div className="view-switcher" role="group" aria-label="Timetable view"><button className={`btn btn--ghost ${view === 'week' ? 'is-active' : ''}`} onClick={() => setView('week')}><CalendarRange size={15}/>Week</button><button className={`btn btn--ghost ${view === 'list' ? 'is-active' : ''}`} onClick={() => setView('list')}><List size={15}/>List</button>{canManageSelectedTerm && <button className="btn btn--primary" onClick={() => setCreateOpen(true)}><Plus size={16}/>New entry</button>}</div>}/>
    {message && <SuccessNote>{message}</SuccessNote>}
    {actionError && <ErrorNote>{actionError}</ErrorNote>}
    {termAware && <div className="toolbar timetable-toolbar timetable-toolbar--term"><Select aria-label="Academic term" value={effectiveTermId ? String(effectiveTermId) : ''} onChange={(e) => setSelectedTermId(Number(e.target.value))} disabled={terms.loading || !terms.data?.terms.length}><option value="" disabled>{terms.loading ? 'Loading terms…' : 'Select term'}</option>{terms.data?.terms.map((term) => <option key={term.id} value={term.id}>{term.name} - {term.status}</option>)}</Select>{selectedTerm && <span className="muted">{selectedTerm.code} - {selectedTerm.status}{selectedTerm.status === 'archived' ? ' - read-only' : ''}</span>}</div>}
    {termAware && terms.error && <ErrorNote>{terms.error}</ErrorNote>}
    {user?.role === 'faculty' && <Section title="Open timetable windows" className="free-slots-section">{freeSlots.loading ? <LoadingState label="Loading open windows"/> : freeSlots.error ? <ErrorState message={freeSlots.error} retry={freeSlots.reload}/> : freeSlots.data ? <><p className="muted free-slots-note">{freeSlots.data.note}</p>{freeSlots.data.slots.length ? <div className="free-slot-grid">{freeSlots.data.slots.map((slot) => <article className="free-slot-card" key={`${slot.day}-${slot.start_time}-${slot.end_time}`}><div className="free-slot-card__icon"><Clock3 size={16}/></div><div><strong>{slot.day}</strong><span>{formatClock(slot.start_time)} - {formatClock(slot.end_time)}</span></div><small>{slot.duration_minutes} min</small></article>)}</div> : <EmptyState title="No open timetable windows" description="No institutional timetable gap meets the minimum duration for this term."/>}</> : null}</Section>}
    <div className="toolbar timetable-toolbar"><label className="search-box"><Search size={17}/><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search course, room, faculty or section"/></label>{canManageSelectedTerm && <label className={`btn btn--secondary file-button ${importing ? 'is-disabled' : ''}`} aria-disabled={importing}><FileUp size={16}/>{importing ? 'Importing…' : 'Import CSV/XLSX'}<input type="file" accept=".csv,.xlsx" disabled={importing} onChange={(e) => { void upload(e.target.files?.[0]); e.currentTarget.value = '' }}/></label>}</div>
    {source.loading ? <LoadingState label="Loading timetable"/> : source.error ? <ErrorState message={source.error} retry={source.reload}/> : filtered.length ? view === 'week' ? <TimetableView entries={filtered}/> : <Section><div className="table-wrap"><table className="data-table"><thead><tr><th>Course</th><th>Day / Time</th><th>Section</th><th>Faculty</th><th>Room</th><th>Type</th><th>Source</th>{canManageSelectedTerm && <th>Actions</th>}</tr></thead><tbody>{filtered.map((entry) => <tr key={entry.id}><td><strong>{classLabel(entry)}</strong><small>{entry.course_code || 'Special event'}</small></td><td>{entry.day}<small>{formatClock(entry.start_time)} – {formatClock(entry.end_time)}</small></td><td>{entry.section || 'Shared'}</td><td>{entry.faculty || '—'}</td><td>{entry.room || 'TBA'}</td><td>{titleCase(entry.class_type)}</td><td><StatusBadge tone={entry.source === 'generated' ? 'success' : 'neutral'}>{titleCase(entry.source)}</StatusBadge></td>{canManageSelectedTerm && <td><div className="row-actions"><button className="icon-btn" disabled={deletingId !== null} onClick={() => setRoomEntry(entry)} aria-label="Change room"><PencilLine size={16}/></button><button className="icon-btn" disabled={deletingId !== null} onClick={() => setTimeEntry(entry)} aria-label="Change day and time"><Clock3 size={16}/></button><button className="icon-btn icon-btn--danger" disabled={deletingId !== null} onClick={() => void remove(entry.id)} aria-label="Delete entry"><Trash2 size={16}/></button></div></td>}</tr>)}</tbody></table></div></Section> : <EmptyState title="No timetable entries found" description={query ? 'Try a broader search.' : 'No schedule entries are available for this view.'}/>}
