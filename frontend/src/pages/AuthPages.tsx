...
function publicStudentRegistrationEnabled() {
  return import.meta.env.VITE_ALLOW_PUBLIC_STUDENT_REGISTRATION === '1'
}
...
export function LoginPage() {
  ...
  const registrationEnabled = publicStudentRegistrationEnabled()
  ...
  return (
    <AuthFrame
      title="Welcome back"
      subtitle="Sign in with your email or student registration number."
    >
      {registered && (
        <SuccessNote>Student account created. Sign in to continue.</SuccessNote>
      )}
      ...
      {registrationEnabled ? (
        ...
        <p className="auth-switch">
          Students receive sign-in credentials from the university.
        </p>
      )}
    </AuthFrame>
  )
...
export function RegisterPage() {
  ...
  if (user) return <Navigate to="/dashboard" replace/>
  if (!publicStudentRegistrationEnabled()) return <Navigate to="/login" replace/>
